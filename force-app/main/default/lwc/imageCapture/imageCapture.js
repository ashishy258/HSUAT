...
import getWorkOrderDetails from "@salesforce/apex/ImageCaptureController.getWorkOrderDetails";
...
export default class ImageCapture extends NavigationMixin(LightningElement) {
  ...
  selectedImageInfo;
  typeOfImageValue;
  notAllowedToUploadMessage;
  imageComment;

  get isAllowedToUploadImage() {
    return this.notAllowedToUploadMessage === null;
  }

  get isTypeOfImageSelected() {
    return this.typeOfImageValue != null;
  }
    ...
    getWorkOrderDetails({recordId : this.recordId})
      .then((result) => {
        if(result['Status'] === 'New'){
          this.notAllowedToUploadMessage = 'It is not possible to upload images to this Work Order as the Service Appointment status is not On-Site';
        }
        else if(result['Status'] === 'In Progress' && result['Process_Payments__c'] === true){
          this.notAllowedToUploadMessage = 'It is not possible to upload images to this Work Order as the request for Payment Processing has already been submitted';
        }
        else if(result['Status'] === 'Completed'){
          this.notAllowedToUploadMessage = 'It is not possible to upload images to this Work Order as it has already been Completed';
        } 
        else{
          this.notAllowedToUploadMessage = null;
        }
        this.template.querySelector('.errorBox').classList.add('slds-box');
        this.isReading = false;    
      })
      .catch((error) => {
        this.isReading = false;
        this.handleErrorConfirm("Either you are offline or something went wrong, Please contact your admin.")
        log(`Failure: ${JSON.stringify(error)}`);
      });
  }
  ...
  readMetadata(file) {
    return new Promise((resolve) => {
      ...
      const metadata = {
        fileName: `${fileNameWithoutExt}__${now}`,
        ext: ext,
        edited: false
      };
  ...
  handleFacePlateImageSelected(event){
    this.typeOfImageValue = event.detail;
  }

  handleRepairImageSelected(event){
    this.typeOfImageValue = event.detail;
  }
  ...
  async handleUploadRequested(event) {
    this.imageComment = event.detail;
    this.toastType = null;
  ...
  async uploadData(fileName, fileId, data, recordId) {
    ...
    let ImageTypeValue=this.typeOfImageValue;
    ...
    const contentVersion = await createRecord({
      ...
      fields: {
        ...
        //Image_Type__c: this.typeOfImageValue,
        Description: this.imageComment
      }
    });

    // Create a ContentDocumentLink (CDL) to associate the uploaded file
    // to the Files Related List of a record, like a Work Order.
    await this.getIdsAndCreateCdl(contentVersion, recordId, uniqueCvId,ImageTypeValue);
  }

  async getIdsAndCreateCdl(contentVersion, recordId, uniqueCvId,ImageTypeValue) {
    ...
    await this.createCdl(recordId, contentDocumentId,ImageTypeValue);
  }
  ...
  async createCdl(recordId, contentDocumentId,ImageTypeValue) {
    log("Creating a CDL...");
  ...
  async handleErrorConfirm(msg) {
    const result = await LightningConfirm.open({
      message: msg,
      variant: "header",
      label: "Add images to record failed",
      theme: "error"
    });
